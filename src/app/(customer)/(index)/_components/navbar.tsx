    ...
    <nav className="container max-w-[1130px] mx-auto flex items-center justify-between bg-[#0D5CD7] p-5 rounded-3xl">
      ...
      <div className="flex items-center gap-3">
        ...
        {session && user.role === 'customer' ? (
          ...
          <>
            <Link href="/sign-in" className="p-[12px_20px] bg-white rounded-full font-semibold text-black">
              Sign In
            </Link>
            <Link href="/sign-up" className="p-[12px_20px] bg-white rounded-full font-semibold text-black">
              Sign Up
            </Link>
          </>
        )}
